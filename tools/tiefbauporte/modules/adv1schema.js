...
function buildEnergySchemaForStrand(strand, opts = {}){
  ...
  // Eintrittspunkt der Leitungen:
  //  - direct:  rechts vom Mast (srcRightX, srcCenterY)
  //  - dist:    KVS-Box, mittig zwischen Quelle und Verbrauchern
  let feedX, feedY, feedExits, feedKind;
  let kvsLeftX, kvsTop, kvsBot;
  ...
  if (strand.kind === 'direct'){
    ...
    const COL_KVS_X = 660;
    kvsLeftX = COL_KVS_X;
    kvsTop = distCY - KVS_H/2;
    kvsBot = distCY + KVS_H/2;
  ...
  // Spuren (lanes) zwischen feedX und COL_CHG_X.
  // Wir verteilen die Verbraucher auf "oben" und "unten" relativ zur feedY.
  // Ohne Splitter: alle gehen auf dieselbe Spur. Mit n Verbrauchern werden
  // n-1 Stufen gebaut, gleichmäßig auf den Korridor verteilt.
  const corridorStart = feedX + 30;            // Leitung tritt 30px nach feedX heraus
  const corridorEnd   = COL_CHG_X - 30;        // letzte 30px laufen horizontal in den Charger
  const corridorW     = corridorEnd - corridorStart;
  const lanesN        = consumers.length;
  // Lanes vom äußersten zum innersten:
  // Verbraucher werden so geordnet, dass der oberste die kürzeste obere Lane
  // bekommt und der unterste die kürzeste untere Lane (wie im Beispielbild).
  // Praktisch: Lane-X = corridorEnd - laneOffset, wobei laneOffset wächst,
  // je weiter der Verbraucher von feedY entfernt ist.

  // Reihenfolge der Verbraucher von außen nach innen (max distanz zuerst)
  const idxByDistance = consumers
    .map((_, i) => ({ i, d: Math.abs(consumerYs[i] - feedY) }))
    .sort((a,b) => b.d - a.d)
    .map(x => x.i);

  // Lane-Offsets so verteilen, dass die innerste Lane möglichst nah am Charger liegt
  // und die äußerste Lane möglichst nah am Speisepunkt.
  const laneSpacing = lanesN > 1 ? corridorW / lanesN : corridorW / 2;
  const laneXForConsumer = new Array(lanesN);
  idxByDistance.forEach((cIdx, rank) => {
    // rank=0 → äußerste (am weitesten weg vom Charger): lane nahe feed
    // rank=lanesN-1 → innerste: lane nahe Charger
    const x = corridorStart + (rank + 0.5) * laneSpacing;
    // Bei einzelnen Lanes: zentriert
    laneXForConsumer[cIdx] = lanesN === 1 ? (corridorStart + corridorEnd) / 2 : x;
  });

  // Verbraucher rendern und Leitungen zeichnen
  consumers.forEach((cons, i) => {
    ...
    // Pfad: (feedX, feedY) → laneX,feedY → laneX,cy → COL_CHG_X,cy
    let pts;
    if (Math.abs(cy - feedY) < 2){
      // gleiche Höhe: gerade Linie
      pts = `${feedX},${feedY} ${COL_CHG_X},${cy}`;
    } else {
      pts = `${feedX},${feedY} ${laneX},${feedY} ${laneX},${cy} ${COL_CHG_X},${cy}`;
    }
    s += `<polyline points="${pts}" fill="none" stroke="${C_LINE_E}" stroke-width="2"/>\n`;

    // Beschriftung: Querschnitt mittig oberhalb der horizontalen Strecke zum Charger
    // Position: mittig zwischen laneX und COL_CHG_X, auf cy
...
function buildLogicalNetworkSchema(opts = {}){
  ...
  // Anchor-Punkt für Edge (rechts/links Mitte des Knotens)
  function rightAnchor(p){
    const n = p.node;
    if (isConsumer(n)) return { x: p.x, y: p.y };
  ...
  // Edges zeichnen (zuerst, damit Knoten oben liegen)
  edges.forEach(e => {
    const a = posMap.get(e.from.id);
    const b = posMap.get(e.to.id);
    if (!a || !b) return;
    const A = rightAnchor(a);
    const B = leftAnchor(b);
    const midX = (A.x + B.x) / 2;
    let pts;
    if (Math.abs(A.y - B.y) < 4){
      pts = `${A.x},${A.y} ${B.x},${B.y}`;
    } else {
      pts = `${A.x},${A.y} ${midX},${A.y} ${midX},${B.y} ${B.x},${B.y}`;
    }
    ...
    s += `<text x="${midX}" y="${(A.y + B.y)/2 - 6}" text-anchor="middle" font-size="11" fill="${C_INK}">${escapeXml(lbl)}</text>\n`;
  });
