...
import { getSupplies, removeSupply } from './links.js';
...
function renderObjectInspector(ctx, el){
  ...
  el.innerHTML = `
    ...
    <div class="insp-h">Erdung</div>
    <div id="iEarthStatus"></div>

    <div class="insp-h">Versorgungen (Einspeisungen)</div>
    <div id="iSuppliesList"></div>
    <button id="iAutoFeed" style="width:100%;margin-top:6px;padding:8px;background:var(--navy,#1B2D5E);color:#fff;border:none;border-radius:5px;font-weight:700;font-size:12px;cursor:pointer">⚡ Automatisch Einspeisen…</button>
    <button id="iAddSupply" style="width:100%;margin-top:4px;padding:8px;background:var(--green);color:#fff;border:none;border-radius:5px;font-weight:700;font-size:12px;cursor:pointer">+ Einspeisung manuell hinzufügen…</button>

    <div class="insp-h">Verknüpfte Trassen (manuell)</div>
    <div class="field">
      <label>Zusätzliche Trassen-Verknüpfung (z.B. Erdungsband)</label>
      <select id="iLinkedTrace">
        <option value="">— keine —</option>
        ${state.traces.map((t,i) => {
          const len = t.segments.reduce((s,sg)=>s+sg.len,0);
          return `<option value="${t.id}" ${o.linkedTraceId===t.id?'selected':''}>${escapeHtml(t.name||'Trasse #'+(i+1))} · ${len.toFixed(1)} m</option>`;
        }).join('')}
      </select>
    </div>
    ...
    <div class="insp-h is-price">Σ Position</div>
    <div class="is-price" style="background:var(--navy);color:#fff;padding:10px 12px;border-radius:6px;text-align:right;font-weight:700;font-size:14px">${fmtEur(sumPrice)}</div>
  ...
  // Erdung-Status
  renderEarthStatus(ctx, o, el.querySelector('#iEarthStatus'));
  // Supplies list
  renderSuppliesList(ctx, o, el.querySelector('#iSuppliesList'));
  el.querySelector('#iAddSupply').onclick = () => {
    import('./feed.js').then(m => m.startFeed(ctx, o.id));
  };
  el.querySelector('#iAutoFeed').onclick = () => {
    import('./feed.js').then(m => m.openAutoFeedDialog(ctx, o.id));
  };
  ...
  el.querySelector('#iDelete').onclick = () => {
    ...
    pushUndo();
    const id = o.id;
    const i = state.objects.findIndex(x => x.id === id);
    if (i>=0){
      state.objects.splice(i,1);
      import('./links.js').then(m => { m.cleanupAfterObjectDelete(id); ctx.refresh(); ctx.save(); });
    }
    ctx.selection = null;
    ctx.refresh(); ctx.save();
  };
...
function renderTraceInspector(ctx, el){
  ...
  el.innerHTML = `
    ...
    <div class="insp-h is-price">Σ Trasse</div>
    <div class="is-price" style="background:var(--navy);color:#fff;padding:10px 12px;border-radius:6px;font-size:12px;line-height:1.5">
  ...
  el.querySelector('#iTDelete').onclick = () => {
    ...
    pushUndo();
    const id = t.id;
    const idx = state.traces.findIndex(x => x.id === id);
    if (idx>=0){
      state.traces.splice(idx,1);
      import('./links.js').then(m => { m.cleanupAfterTraceDelete(id); ctx.refresh(); ctx.save(); });
    }
    ctx.selection = null;
    ctx.refresh(); ctx.save();
  };
  ...
  if (segIdx != null){
    ...
    const segDel = el.querySelector('#iSegDel');
    if (segDel) segDel.onclick = () => {
      if (t.segments.length <= 1){
        ctx.showToast('Letztes Segment kann nicht gelöscht werden — Trasse löschen statt dessen', 'warn');
        return;
      }
      const which = t.points.length > 2
        ? confirm(`Segment #${segIdx+1} löschen?\n\nOK = Endpunkt entfernen (Punkt ${segIdx+2})\nAbbrechen = Startpunkt entfernen (Punkt ${segIdx+1})`)
        : true;
      pushUndo();
      const removeIdx = which ? segIdx+1 : segIdx;
      t.points.splice(removeIdx, 1);
      t.segments.splice(segIdx, 1);
      (t.cables||[]).forEach(c => {
        c.segIds = (c.segIds||[])
          .filter(i => i !== segIdx)
          .map(i => i > segIdx ? i-1 : i);
      });
      // Asset-linkedPointIdx anpassen
      state.objects.forEach(o => {
        if (o.linkedTraceId === t.id && o.linkedPointIdx != null){
          if (o.linkedPointIdx === removeIdx) o.linkedPointIdx = null;
          else if (o.linkedPointIdx > removeIdx) o.linkedPointIdx -= 1;
        }
      });
      recalcSegments(t);
      // Alle supplies neu rechnen, da sich Punkt-Indices verschoben haben
      import('./links.js').then(m => { m.recomputeAllSupplies(); ctx.refresh(); ctx.save(); });
      ctx.selection = { kind:'trace', id:t.id };
      ctx.refresh(); ctx.save();
      ctx.showToast(`Segment gelöscht`, 'ok');
    };
  }
}
...
function renderSegmentEditor(t, segIdx){
  ...
  return `
    ...
    <div style="display:flex;gap:6px;margin-top:6px">
      <button id="iSegDel" style="flex:1;padding:6px 10px;border:1px solid #c33;background:#fff;color:#c33;border-radius:4px;font-size:11px;font-weight:600;cursor:pointer">Segment löschen</button>
    </div>
  `;
}
...
function renderEarthStatus(ctx, obj, container){
  const earth = obj.earth;
  if (!earth || !earth.cableTypeId){
    container.innerHTML = `
      <div style="background:#fff5f5;border:1px solid #ffd0d0;border-radius:6px;padding:8px 10px;font-size:11px;color:#a33;display:flex;align-items:center;gap:8px">
        <span style="font-size:16px">⚠</span>
        <span style="flex:1">Kein Erdungsabzweig vorhanden</span>
        <button id="iEarthAdd" style="background:#fff;border:1px solid #c33;color:#c33;border-radius:4px;padding:4px 8px;font-size:11px;font-weight:600;cursor:pointer">+ hinzufügen</button>
      </div>`;
    const addBtn = container.querySelector('#iEarthAdd');
    if (addBtn) addBtn.onclick = () => import('./feed.js').then(m => m.openAutoFeedDialog(ctx, obj.id));
    return;
  }
  const ct = state.cableTypes.find(c => c.id === earth.cableTypeId);
  const len = Number(earth.length)||0;
  const cc = Number(earth.crossConnector)||0;
  container.innerHTML = `
    <div style="background:#eaf6ee;border:1px solid #00695C;border-radius:6px;padding:8px 10px;font-size:11px;color:#1c3a2a">
      <div style="display:flex;align-items:center;gap:6px;font-weight:700">
        <span style="font-size:14px">⚓</span>
        <span style="flex:1">Erdungsabzweig vorhanden</span>
        <button id="iEarthDel" title="Erdung entfernen" style="background:#fff;border:1px solid var(--red,#D32F2F);color:var(--red,#D32F2F);border-radius:3px;width:22px;height:22px;cursor:pointer">×</button>
      </div>
      <div style="margin-top:4px;line-height:1.5">
        ${ct ? `<span style="display:inline-block;width:10px;height:10px;border-radius:2px;background:${ct.color};vertical-align:middle;margin-right:4px"></span>${escapeHtml(ct.label)}` : '—'}
        · <b>${len} m</b>${cc ? ` · ${cc}× Kreuzverbinder` : ''}
      </div>
    </div>`;
  const del = container.querySelector('#iEarthDel');
  if (del) del.onclick = () => {
    if (!confirm('Erdung entfernen?')) return;
    pushUndo();
    delete obj.earth;
    ctx.refresh(); ctx.save();
  };
...
function renderSuppliesList(ctx, obj, container){
  const supplies = getSupplies(obj);
  if (!supplies.length){
    container.innerHTML = `<div style="background:var(--bg-2);padding:10px;border-radius:5px;font-size:11px;color:var(--ink-3);text-align:center;line-height:1.5">
      Keine Einspeisung definiert.<br>
      <small>Schränke, Verbraucher oder weitere Schränke können als Quelle dienen.</small>
    </div>`;
    return;
  }
  container.innerHTML = supplies.map(s => {
    const src = state.objects.find(x => x.id === s.sourceId);
    const srcCat = src ? state.catalog.find(c => c.id === src.catId) : null;
    const cable = state.cableTypes.find(c => c.id === s.cableTypeId);
    const traces = (s.traceIds||[]).map(tid => state.traces.find(t => t.id === tid)).filter(Boolean);
    // Tatsächliche Pfadlänge aus den Cable-segIds (nicht Volltrasse)
    let totalLen = 0;
    traces.forEach(t => {
      const cab = (t.cables||[]).find(c => c.feedSupplyId === s.id);
      if (cab){
        totalLen += (cab.segIds||[]).reduce((sg, i) => sg + (t.segments[i]?.len||0), 0);
      }
    });
    const eff = s.reserveMode === 'm' ? totalLen + (Number(s.reserveValue)||0) : totalLen * (1 + (Number(s.reserveValue)||0)/100);
    const meters = eff * (Number(s.count)||1);
    const cost = meters * (cable?.price || 0);
    return `
      <div data-supply="${s.id}" style="border:1px solid var(--line);border-radius:6px;padding:8px;margin-bottom:6px;background:#fff">
        <div style="display:flex;align-items:center;gap:6px;margin-bottom:4px">
          <span style="font-size:14px">${srcCat?.icon||'?'}</span>
          <b style="flex:1;font-size:12px">${escapeHtml(src ? (src.customName||srcCat?.name||'?') : '— Quelle fehlt —')}</b>
          <button data-act="edit" title="Bearbeiten" style="background:#fff;border:1px solid var(--line);border-radius:3px;width:24px;height:24px;cursor:pointer">✎</button>
          <button data-act="del" title="Entfernen" style="background:#fff;border:1px solid var(--red, #D32F2F);color:var(--red,#D32F2F);border-radius:3px;width:24px;height:24px;cursor:pointer">×</button>
        </div>
        <div style="font-size:11px;color:var(--ink-2);line-height:1.5">
          ${cable ? `<span style="display:inline-block;width:10px;height:10px;border-radius:2px;background:${cable.color};vertical-align:middle;margin-right:4px"></span><b>${escapeHtml(cable.label)}</b>` : '<i>kein Kabel</i>'}
          ${s.count > 1 ? ` · ${s.count}×` : ''}
          ${traces.length ? ` · über ${traces.length} Trasse${traces.length>1?'n':''}: ${traces.map((t,i)=>{
            const idx = state.traces.indexOf(t);
            return escapeHtml(t.name?.trim() || `Trasse ${idx+1}`);
          }).join(' → ')}` : ' · keine Trasse'}
          ${cable && totalLen ? `<br><span style="color:var(--ink-3)">Σ ${meters.toFixed(1)} m · ${fmtEur(cost)}</span>` : ''}
        </div>
      </div>
    `;
  }).join('');
  container.querySelectorAll('[data-supply]').forEach(card => {
    const sid = card.dataset.supply;
    card.querySelector('[data-act="edit"]').onclick = () => {
      import('./feed.js').then(m => m.startFeed(ctx, obj.id, sid));
    };
    card.querySelector('[data-act="del"]').onclick = () => {
      if (!confirm('Einspeisung entfernen (inkl. Kabel auf Trassen)?')) return;
      pushUndo();
      removeSupply(obj, sid);
      // Spiegelfelder bereinigen wenn keine supplies mehr
      if (!obj.supplies.length){ obj.supplyFromId = null; obj.supplyCable = false; }
      ctx.refresh(); ctx.save();
    };
  });
}
