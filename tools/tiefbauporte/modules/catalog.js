...
export function renderCatalog(ctx){
  ...
  list.querySelectorAll('.cat-item').forEach(el => {
    el.ondblclick = () => {
      // Rechte Spalte ausklappen, falls ausgeblendet, und Inspector zeigen
      ...
      // Inspector-Tab aktivieren
      const insTab = document.querySelector('.col-right .ptab[data-pt="inspector"]');
...
function renderCatItem(c){
  ...
  return `<div class="cat-item ${sel?'sel':''}" data-id="${c.id}">
    ...
    <div class="meta">
      ...
      <div class="sub">${c.pos ? 'LV '+c.pos+' · ' : ''}<span class="is-price">${fmtEur(c.price)}</span></div>
    </div>
