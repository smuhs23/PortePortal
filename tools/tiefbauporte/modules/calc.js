...
// v6.1: Effektive Kabelliste für ein Segment ermitteln (Mischmodus)
//   seg.cables === undefined  → erbe von Trasse (t.cables)
//   seg.cables === []         → Override "leer" (keine Kabel)
//   seg.cables === [{...}]    → eigene Belegung
export function getEffectiveCablesForSegment(t, seg) {
  if (seg.cables === undefined || seg.cables === null) {
    return { cables: t.cables || [], source: 'inherited' };
  }
  if (Array.isArray(seg.cables) && seg.cables.length === 0) {
    return { cables: [], source: 'empty' };
  }
  return { cables: seg.cables, source: 'own' };
}

// Hat irgendein Segment einen Override?
export function hasSegmentOverrides(t) {
  return (t.segments || []).some(seg => seg.cables !== undefined && seg.cables !== null);
}

// Kabel-Kosten einer Trasse berechnen (Mischmodus)
// Wenn keine Segment-Overrides existieren → klassisch (Trassen-Default × Gesamtlänge)
// Wenn Overrides existieren → pro Segment summieren
function calcTraceCableCost(t) {
  const overrides = hasSegmentOverrides(t);
  if (!overrides) {
    // Klassisch: Default-Belegung über Gesamtlänge
    ...
    (t.cables || []).forEach(c => {
      ...
      total += cost;
      breakdown.push({
        typeId: c.typeId, label: c.label, count: n,
        reserveMode: c.reserveMode, reserveValue: c.reserveValue,
        effLen, unitPrice, priceOverride: c.priceOverride, cost,
        scope: 'trace'
      });
    });
  ...
  // Mischmodus: pro Segment summieren
  let total = 0;
  // Zwischen-Aggregation: gleiche typeId zusammenfassen für Breakdown
  const agg = new Map();
  (t.segments || []).forEach((seg, segIdx) => {
    const { cables, source } = getEffectiveCablesForSegment(t, seg);
    cables.forEach(c => {
      const n = Number(c.count) || 0;
      if (n <= 0) return;
      const effLen = cableEffectiveLength(c, seg.len);
      const unitPrice = cableUnitPrice(c);
      const cost = effLen * n * unitPrice;
      total += cost;
      const key = `${c.typeId}|${unitPrice}|${c.reserveMode}|${c.reserveValue}|${n}`;
      if (!agg.has(key)) {
        agg.set(key, {
          typeId: c.typeId, label: c.label, count: n,
          reserveMode: c.reserveMode, reserveValue: c.reserveValue,
          effLen: 0, unitPrice, priceOverride: c.priceOverride, cost: 0,
          segments: []
        });
      }
      const e = agg.get(key);
      e.effLen += effLen;
      e.cost += cost;
      e.segments.push(segIdx + 1);
    });
  });
  return { total, breakdown: Array.from(agg.values()) };
}
...
export function calcTotals(state) {
  ...
  state.traces.forEach(t => {
    ...
    // v6.1: Mischmodus-Kabelberechnung (Segment-Overrides werden berücksichtigt)
