...
export function renderBOM(ctx){
  ...
  if (assetMap.size){
    html += `<div class="bom-section">
      ...
      <table class="bom-table">
        <thead><tr><th>Pos.</th><th>Beschreibung</th><th class="r">Anzahl</th><th class="r col-price">EP</th><th class="r col-price">Σ</th></tr></thead>
        <tbody>
          ${[...assetMap.values()].map(v => `<tr>
            <td>${escapeHtml(v.pos||'-')}</td>
            <td>${escapeHtml(v.name)}</td>
            <td class="r">${v.count} ${v.unit}</td>
            <td class="r col-price">${fmtEur(v.sum / Math.max(v.count,1))}</td>
            <td class="r col-price">${fmtEur(v.sum)}</td>
          </tr>`).join('')}
        </tbody>
        <tfoot><tr><td colspan="4">Σ Hardware</td><td class="r col-price">${fmtEur(assetSum)}</td></tr></tfoot>
      </table>
    </div>`;
  }
  ...
  if (state.traces.length){
    html += `<div class="bom-section">
      ...
      <table class="bom-table">
        <thead><tr><th>Oberfläche</th><th class="r">Meter</th><th class="r col-price">€/m</th><th class="r col-price">Σ</th></tr></thead>
        <tbody>
          ${Object.entries(trenchByOF).map(([of, m]) => {
            ...
            return `<tr>
              ...
              <td class="r col-price">${epM.toFixed(2)}</td>
              <td class="r col-price">${fmtEur(m*epM)}</td>
            </tr>`;
          }).join('')}
        </tbody>
        <tfoot><tr><td colspan="3">Σ Tiefbau</td><td class="r col-price">${fmtEur(trenchTotal)}</td></tr></tfoot>
      </table>
    </div>`;
  }
  ...
  if (cableMap.size){
    html += `<div class="bom-section">
      ...
      <table class="bom-table">
        <thead><tr><th>Typ</th><th class="r">Stk</th><th class="r">Meter</th><th class="r col-price">EP</th><th class="r col-price">Σ</th></tr></thead>
        <tbody>
          ${[...cableMap.values()].map(v => `<tr>
            <td>${escapeHtml(v.label)}</td>
            <td class="r">${v.totalCount}</td>
            <td class="r"><b>${v.totalMeters.toFixed(1)}</b></td>
            <td class="r col-price">${v.unitPrice.toFixed(2)}</td>
            <td class="r col-price">${fmtEur(v.totalCost)}</td>
          </tr>`).join('')}
        </tbody>
        <tfoot><tr><td colspan="4">Σ Kabel</td><td class="r col-price">${fmtEur(cableSum)}</td></tr></tfoot>
      </table>
    </div>`;
  }
  ...
  if (earthMap.size || crossConnectorCount){
    html += `<div class="bom-section">
      ...
      <table class="bom-table">
        <thead><tr><th>Typ</th><th class="r">Meter / Stk</th><th class="r col-price">EP</th><th class="r col-price">Σ</th></tr></thead>
        <tbody>
          ...
          ${crossConnectorCount && kreuzCat ? `<tr>
            ...
            <td class="r col-price">${kreuzCat.price.toFixed(2)}</td>
            <td class="r col-price">${fmtEur(crossSum)}</td>
          </tr>` : ''}
        </tbody>
        <tfoot><tr><td colspan="3">Σ Erdung</td><td class="r col-price">${fmtEur(earthSum)}</td></tr></tfoot>
      </table>
    </div>`;
  }

  // Totals
  html += `<div class="bom-section" data-bom-priceonly>
