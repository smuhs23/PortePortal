  ...
  if (!t.cables || !t.cables.length){
    ...
    t.cables.forEach((cab, ci) => {
      ...
      row.innerHTML = `
        ...
        <div class="meta">
          <b>${meters.toFixed(1)} m</b>
          <div class="reserve-edit" title="Reserve / Schleifenzugabe">
            <input class="rv" type="number" step="0.5" min="0" value="${cab.reserveValue}">
            <select class="rm">
      ...
      // Reserve-Editor
      const rvInp = row.querySelector('.rv');
      const rmSel = row.querySelector('.rm');
      const commitReserve = () => {
        const v = Math.max(0, Number(rvInp.value)||0);
        const m = rmSel.value === 'm' ? 'm' : 'pct';
        if (cab.reserveValue === v && cab.reserveMode === m) return;
        pushUndo();
        cab.reserveValue = v;
        cab.reserveMode = m;
        renderTimeline(ctx, t);
      ...
      rvInp.onchange = commitReserve;
      rmSel.onchange = commitReserve;
      // Live-Preview ohne speichern
      rvInp.oninput = () => {
        const v = Math.max(0, Number(rvInp.value)||0);
        const baseLen = (cab.segIds||[]).reduce((s,i) => s + (t.segments[i]?.len||0), 0);
        const eff = rmSel.value === 'm' ? baseLen + v : baseLen * (1 + v/100);
        const m = eff * (Number(cab.count)||0);
        row.querySelector('.meta b').textContent = m.toFixed(1) + ' m';
      };
...
function openAddCable(ctx, t){
    ...
    <div class="modal-backdrop" id="mB">
      <div class="modal">
        ...
        <div class="body">
          ...
          <div id="cablePickList" style="display:flex;flex-direction:column;gap:6px;max-height:50vh;overflow-y:auto"></div>
        </div>
  ...
  // Gruppierte Kabelliste rendern (Daten / Leerrohr / Cu / Al / Erdung)
  const groupsOrder = [
  ...
  const openGroups = new Set(['data','pipe']);
  const list = root.querySelector('#cablePickList');
  function renderList(){
    ...
    list.innerHTML = groupsOrder.filter(g => byGroup.has(g.id)).map(g => {
      const items = byGroup.get(g.id);
      const open = openGroups.has(g.id);
      return `
        <div style="border:1px solid var(--line);border-radius:5px;overflow:hidden">
          <button data-grp="${g.id}" class="cable-grp-h" style="width:100%;display:flex;align-items:center;gap:6px;padding:8px 10px;background:#fafafa;border:none;cursor:pointer;font-size:12px;font-weight:600;text-align:left">
            <span style="display:inline-block;width:10px">${open?'▾':'▸'}</span>
            <span style="flex:1">${escapeHtml(g.label)}</span>
            <span style="color:var(--ink-3);font-weight:400;font-size:11px">${items.length}</span>
          </button>
          <div style="display:${open?'flex':'none'};flex-direction:column;gap:3px;padding:${open?'6px':'0'}">
            ${items.map(c => `
              <button class="cable-pick" data-id="${c.id}" style="display:flex;align-items:center;gap:8px;padding:6px 8px;background:#fff;border:1px solid var(--line);border-radius:4px;cursor:pointer;text-align:left;font-size:12px">
    ...
    list.querySelectorAll('.cable-grp-h').forEach(b => {
      b.onclick = () => {
    ...
    list.querySelectorAll('.cable-pick').forEach(b => {
      b.onclick = () => {
