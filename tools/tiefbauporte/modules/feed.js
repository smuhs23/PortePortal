...
export function openAutoFeedDialog(ctx, consumerId){
  ...
  const powerCables = state.cableTypes.filter(c => ['k1','k2','k3','k4'].includes(c.id) ||
    (String(c.label||'').toLowerCase().includes('strom')));
      ...
      <div class="modal" style="max-width:580px">
        ...
        <div class="body">
          ...
          <select id="afCable" style="width:100%;padding:6px 8px;border:1px solid var(--line);border-radius:4px;font-size:12px;margin-bottom:10px">
            ${powerCables.map(c => `<option value="${c.id}">${escapeHtml(c.label)} – ${c.price.toFixed(2)} €/m</option>`).join('')}
          </select>
