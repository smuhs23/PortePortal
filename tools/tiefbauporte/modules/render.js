...
function renderPins(ctx) {
  ...
  state.objects.forEach(o => {
    ...
    // Pin in einem Hit-Wrapper (44px = Apple HIG Touch-Target) — Pin selbst kompakt
    const pinHtml = `<div class="${cls}" style="background:${s.color}">${inner}</div>${labelHtml}`;

    // v6.1: kleinere Pins per Default; bigPins toggle macht sie wieder größer
    const big = !!state.viz?.bigPins;
    const visualSz = big ? 48 : 34;
    const hitSz = Math.max(visualSz, 44);  // mind. 44 px Touch-Target
    const html = `<div class="pin-hit" style="width:${hitSz}px;height:${hitSz}px;display:flex;align-items:center;justify-content:center;">${pinHtml}</div>`;
    ...
    m.on('dragstart', (e) => {
      // Visuelles Feedback: Marker hervorheben während Drag
      const elDrag = e.target.getElement();
      if (elDrag) elDrag.classList.add('marker-dragging');
    });
    m.on('drag', (e) => {
      // Position live im Tooltip aktualisieren
      const p = e.target.getLatLng();
      const tip = e.target.getTooltip();
      if (tip) {
        tip.setContent(`<b>Position:</b><br>${p.lat.toFixed(6)}, ${p.lng.toFixed(6)}`);
        if (!tip.isOpen()) e.target.openTooltip();
      }
    });
    m.on('dragend', (e) => {
      const p = e.target.getLatLng();
      ...
      const elDrag = e.target.getElement();
      if (elDrag) elDrag.classList.remove('marker-dragging');
      ctx.save();
      // Tooltip wieder mit normalem Inhalt füllen, geschieht beim nächsten Render
      ctx.render();
    });
    ...
    // Tooltip am visuellen Pin-Rand, nicht am Hit-Container-Rand
    const tipOff = big ? -28 : -20;
