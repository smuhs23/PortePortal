...
const APP_VERSION = 'v6.4.4-2026-04-26';
const APP_CACHE = `tbp-app-${APP_VERSION}`;
const TILE_CACHE = `tbp-tiles-v2`;
...
self.addEventListener('activate', (event) => {
  event.waitUntil(
    caches.keys().then(keys => Promise.all(
      keys
        // Alle alten tbp-Caches entsorgen außer den aktuellen
        .filter(k => k.startsWith('tbp-') && k !== APP_CACHE && k !== TILE_CACHE && k !== LIB_CACHE)
