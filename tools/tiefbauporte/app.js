...
function initTopbar(){
  ...
  // Preise ein-/ausblenden (pro Projekt persistiert)
  const applyPriceMode = () => {
    const off = state.viz.showPrices === false;
    document.body.classList.toggle('no-prices', off);
    const btn = document.getElementById('btnTogglePrices');
    if (btn){
      btn.title = off ? 'Preise einblenden' : 'Preise ausblenden';
      btn.textContent = off ? '🔒' : '💰';
    }
  };
  applyPriceMode();
  document.getElementById('btnTogglePrices').onclick = () => {
    state.viz.showPrices = state.viz.showPrices === false ? true : false;
    applyPriceMode();
    saveState();
    fullRender();
  };
  // Reopen tabs am Kartenrand
  const reopenL = document.getElementById('btnReopenLeft');
  const reopenR = document.getElementById('btnReopenRight');
  if (reopenL) reopenL.onclick = () => { document.body.classList.remove('hide-left'); setTimeout(() => ctx.map?.invalidateSize?.(), 220); };
  if (reopenR) reopenR.onclick = () => { document.body.classList.remove('hide-right'); setTimeout(() => ctx.map?.invalidateSize?.(), 220); };
  // Basemap
...
function deleteSelection(){
  ...
  if (ctx.selection.kind === 'object'){
    ...
    import('./modules/links.js').then(m => { m.cleanupAfterObjectDelete(id); fullRender(); ctx.save(); });
  } else if (ctx.selection.kind === 'trace'){
    const id = ctx.selection.id;
    const idx = state.traces.findIndex(t => t.id === id);
    if (idx >= 0) state.traces.splice(idx, 1);
    import('./modules/links.js').then(m => { m.cleanupAfterTraceDelete(id); fullRender(); ctx.save(); });
  }
  ctx.selection = null;
  fullRender();
  ctx.save();
...
async function init(){
  ...
  const vizSL = document.getElementById('vizSupplyLines');
  if (vizSL){
    vizSL.checked = state.viz.supplyLines !== false;
    vizSL.onchange = (e) => { state.viz.supplyLines = e.target.checked; renderPins(ctx); saveState(); };
  }
