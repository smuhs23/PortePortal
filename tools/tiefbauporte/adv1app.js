...
import { calcTotals, migrateState, normalizeCables } from './modules/calc.js';
...
import { openBOM } from './modules/bom.js';
...
// v6.2: Migration v6.1 (seg.cables) → v6.2 (t.cables[].segIds)
migrateState(state);
// Defensiv: Cables normalisieren (IDs vergeben, segIds dedupen, ungültige raus)
state.traces.forEach(t => normalizeCables(t));
save();

// v6.4.3: Auf Leaflet warten — falls async langsam lädt, Bootstrap nicht crashen lassen
...
function showFatalError(msg) {
  // Sichtbare Fehlermeldung in der Map-Box
  const mapEl = document.getElementById('map');
  if (mapEl) {
    mapEl.innerHTML = `
      <div style="padding:30px;text-align:center;color:#1B2D5E;background:#fff;height:100%;display:flex;flex-direction:column;align-items:center;justify-content:center;font-family:'Inter',sans-serif">
        <div style="font-size:42px;margin-bottom:10px">⚠️</div>
        <div style="font-size:18px;font-weight:bold;margin-bottom:8px">Karte kann nicht geladen werden</div>
        <div style="font-size:13px;color:#666;margin-bottom:20px;max-width:400px">${msg}</div>
        <button onclick="location.reload()" style="background:#1B2D5E;color:#fff;border:none;padding:10px 20px;border-radius:8px;font-weight:bold;cursor:pointer;font-size:13px">Neu laden</button>
        <button onclick="(async()=>{try{const r=await navigator.serviceWorker.getRegistrations();for(const x of r)await x.unregister();const k=await caches.keys();for(const c of k)await caches.delete(c);localStorage.clear();location.reload();}catch(e){alert(e)}})()" style="background:#fff;color:#1B2D5E;border:1px solid #1B2D5E;padding:8px 16px;border-radius:8px;font-weight:bold;cursor:pointer;font-size:11px;margin-top:8px">Cache komplett leeren &amp; neu laden</button>
      </div>
    `;
  }
  // Header-Buttons trotzdem klickbar machen — wenn schon ohne Map
  document.querySelectorAll('button').forEach(b => {
    if (!b.onclick) b.onclick = () => alert('Karte ist nicht geladen — bitte zuerst „Neu laden" oben klicken.');
  });
}

// ========== Bootstrap (umhüllt alle Map-abhängige Logik) ==========
(async function bootstrap() {
  const ok = await waitForLeaflet();
  if (!ok) {
    showFatalError('Die Karten-Bibliothek (Leaflet) konnte nicht geladen werden. Prüfe deine Internetverbindung. Falls das Problem bleibt, leere den Cache mit dem Knopf unten.');
    return;
  }
  try {
    runApp();
    ...
    showFatalError('Initialisierungsfehler: ' + (e.message || e));
  }
})();
...
// v6.4.4: OSM-Tile-URL ohne {s}-Subdomain (a/b/c.tile.openstreetmap.de wurden 1.8.2023 abgeschaltet).
//         Wir nutzen jetzt direkt den offiziellen tile.openstreetmap.org Service mit HTTP/2.
...
function updateTotal() {
  ...
  // v6.4: Kabelmeter mit anzeigen
  if (t.sumCableMeters > 0) {
    sub += ` · 📏 ${Math.round(t.sumCableMeters)} m Kabel`;
  }
...
document.getElementById('btnBOM').onclick = () => openBOM(ctx);
...
// Erste Zeichnung
ctx.render();
// v6.4.4: invalidateSize mehrfach aufrufen — bei async Bootstrap-Init hat Leaflet
// manchmal 0×0 als Container-Größe gesehen, was zu grauer/leerer Karte führt
map.invalidateSize();
...
// Auch bei Orientierungswechsel (iPad rotiert) und Tab-Sichtbarkeit
